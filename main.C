...
inline double f(const std::vector<double>& x)
{
...
void applyLaplacian(std::shared_ptr<PatchHierarchy> patch_hierarchy, std::shared_ptr<SideVariable<double>> u_var, const int u_idx, const int r_idx);
...
int main(int argc, char **argv)
{
  ...
  if (argc != 2)
  {
    tbox::pout << "USAGE: " << argv[0] << " <input filename> " << "[options]\n\n";
  ...
  auto grid_geometry = std::make_shared<CartesianGridGeometry>(dim, "CartesianGeometry", input_db->getDatabase("CartesianGeometry"));

  auto patch_hierarchy = std::make_shared<PatchHierarchy>("PatchHierarchy", grid_geometry, input_db->getDatabase("PatchHierarchy"));

  auto visit_data_writer = std::make_shared<VisItDataWriter>(dim, "Oldroyd B Visit Writer", visit_dump_dirname, visit_number_procs_per_file);
 
  auto poisson = std::make_shared<Poisson>("Poisson", dim, nullptr, grid_geometry);

  auto tag_and_init_ops = std::make_shared<StandardTagAndInitialize>("StandardTagAndInitialize", poisson.get(), input_db->getDatabase("StandardTagAndInitialize"));

  // Gridding algorithm stuff...
  auto box_generator = std::make_shared<BergerRigoutsos>(dim);

  auto load_balancer = std::make_shared<ChopAndPackLoadBalancer>(dim, "ChopAndPackLoadBalancer", input_db->getDatabase("LoadBalancer"));

  auto gridding_algorithm = std::make_shared<GriddingAlgorithm>(patch_hierarchy, "GriddingAlgorithm", input_db->getDatabase("GriddingAlgorithm"), tag_and_init_ops, box_generator, load_balancer);
  ...
  bool done = false; bool initial_time = true;
...
void applyLaplacian(std::shared_ptr<PatchHierarchy> patch_hierarchy, std::shared_ptr<SideVariable<double>> u_var, const int u_idx, const int r_idx)
{
    std::shared_ptr<CartesianGridGeometry> grid_geometry = std::static_pointer_cast<CartesianGridGeometry>(patch_hierarchy->getGridGeometry());
    ...
    if (patch_hierarchy->getFinestLevelNumber() > 0)
    {
        ...
        int coarser_level = 0;
        std::shared_ptr<CoarsenOperator> coarsen_op(grid_geometry->lookupCoarsenOperator(u_var, "CONSERVATIVE_COARSEN"));
        ...
        coarsen_sched = coarsen_alg.createSchedule(patch_hierarchy->getPatchLevel(coarser_level), patch_hierarchy->getPatchLevel(fill_level_num));
        coarsen_sched->coarsenData();
        std::shared_ptr<RefineOperator> refine_op(grid_geometry->lookupRefineOperator(u_var, "CONSERVATIVE_LINEAR_REFINE"));
        ...
        fill_ghosts_sched = ghost_fill_alg.createSchedule(patch_hierarchy->getPatchLevel(fill_level_num), coarser_level, patch_hierarchy, nullptr/*patch_strategy*/);
    ...
    for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber(); ++ln)
    {
        ...
        for (auto p = level->begin(); p != level->end(); ++p)
        {
            ...
            for (SideIterator si(SideGeometry::begin(box, 0)); si != send; ++si)
            {
                ...
                t(1) = 1; r(0) = 1;
                (*r_data)(idx) = ((*u_data)(idx+t) + (*u_data)(idx-t) + (*u_data)(idx+r) + (*u_data)(idx-r) - 4*(*u_data)(idx))/(dx[0]*dx[1]);
            }
        }
    }
    return;
...
int getCellWeightPatchDescriptorIndex(std::shared_ptr<PatchHierarchy > patch_hierarchy)
{
   ...
   {
     std::shared_ptr<PatchLevel > level = patch_hierarchy->getPatchLevel(ln);
     if(!level->checkAllocated(wgt_cc_idx))
       level->allocatePatchData(wgt_cc_idx);
